...
public strictfp class Communications {
    ...
    public static final int OVERWRITE_DIST_SQ = 9;
    public static final int UPDATE_DIST_SQ = 10;
    
    public static final int FLAGTAKEN_INDEX = 46;
    public static final int ENEMYFLAG_INDEX = 47;
    ...
    public static final int STOLEN_FLAG_SCORE = 200;
    ...
    public static void updateEnemies(RobotController rc, RobotInfo[] robots, FlagInfo[] myNearbyFlags) throws GameActionException {
        ...
        for (RobotInfo robot : robots) {
            if (robot.team != team) {
                ...
                if (score > targetScore ||
                        (score == targetScore && robot.location.distanceSquaredTo(curr) > target.distanceSquaredTo(curr))) {
                    targetScore = score;
                ...
                totalScore -= 1;
                allies++;
                if (robot.hasFlag) totalScore += HOLDING_FLAG_SCORE;
            }
        }

        if (rc.hasFlag()) {
            totalScore += HOLDING_FLAG_SCORE;
            if (target == null && allies <= 8) {
                target = rc.getLocation();
                //rc.setIndicatorString("SETTING SELF TARGET");
            }
        }
        ...
        for (int i = ENEMIES_START + ENEMIES_NUM * 2 - 2; i >= ENEMIES_START; i -= 2) {
            ...
            if (target != null) {
                if (l.isWithinDistanceSquared(curr, GameConstants.VISION_RADIUS_SQUARED)) {
                    if (totalScore > score) {
    ...
    static MapLocation[] enemyFlagSpawns = new MapLocation[3];
    static boolean[] taken = new boolean[3];

    public static void markTaken(RobotController rc, int id) throws GameActionException {
        if (array[ENEMYFLAG_INDEX] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] | 1;
        } else if (array[ENEMYFLAG_INDEX+2] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] | 2;
        } else if (array[ENEMYFLAG_INDEX+4] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] | 4;
        }
        rc.writeSharedArray(FLAGTAKEN_INDEX, array[FLAGTAKEN_INDEX]);
    }

    public static void markDropped(RobotController rc, int id) throws GameActionException {
        if (array[ENEMYFLAG_INDEX] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] & 6;
        } else if (array[ENEMYFLAG_INDEX+2] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] & 5;
        } else if (array[ENEMYFLAG_INDEX+4] == id) {
            array[FLAGTAKEN_INDEX] = array[FLAGTAKEN_INDEX] & 3;
        }
        rc.writeSharedArray(FLAGTAKEN_INDEX, array[FLAGTAKEN_INDEX]);
    }

    public static void updateEnemyFlags(RobotController rc, FlagInfo[] enemyFlags) throws GameActionException {
        int num = 0;
        if (array[ENEMYFLAG_INDEX] > 0) {
            enemyFlagSpawns[0] = new MapLocation(array[ENEMYFLAG_INDEX+1] >> 6, array[ENEMYFLAG_INDEX+1] % 64);
            taken[0] = (array[FLAGTAKEN_INDEX] & 1) > 0;
            num++;
        }
        if (array[ENEMYFLAG_INDEX+2] > 0) {
            enemyFlagSpawns[1] = new MapLocation(array[ENEMYFLAG_INDEX+3] >> 6, array[ENEMYFLAG_INDEX+3] % 64);
            taken[1] = (array[FLAGTAKEN_INDEX] & 2) > 0;
            num++;
        }
        if (array[ENEMYFLAG_INDEX+4] > 0) {
            enemyFlagSpawns[2] = new MapLocation(array[ENEMYFLAG_INDEX+5] >> 6, array[ENEMYFLAG_INDEX+5] % 64);
            taken[2] = (array[FLAGTAKEN_INDEX] & 4) > 0;
            num++;
        }
        for (int i = 3; i-->0;) {
            if (enemyFlagSpawns[i] != null) {
                if (taken[i]) {
                    rc.setIndicatorDot(enemyFlagSpawns[i], 0, 0, 255);
                } else {
                    rc.setIndicatorDot(enemyFlagSpawns[i], 0, 255, 0);
                }
            }
        }
        if (num >= 3) {
        ...
        for (FlagInfo info : enemyFlags) {
            ...
            if (array[ENEMYFLAG_INDEX] == info.getID() || array[ENEMYFLAG_INDEX+2] == info.getID() || array[ENEMYFLAG_INDEX+4] == info.getID()) {
                continue;
            }
            empty = -1;
            if (array[ENEMYFLAG_INDEX] == 0) {
                empty = ENEMYFLAG_INDEX;
            } else if (array[ENEMYFLAG_INDEX+2] == 0) {
                empty = ENEMYFLAG_INDEX+2;
            } else if (array[ENEMYFLAG_INDEX+4] == 0) {
                empty = ENEMYFLAG_INDEX+4;
            }
            if (empty >= 0) {
                array[empty] = info.getID();
                array[empty+1] = (info.getLocation().x * 64) + info.getLocation().y;
                rc.writeSharedArray(empty, array[empty]);
                rc.writeSharedArray(empty+1, array[empty+1]);
            } else {
                full = true;
            }
        }
    ...
    public static MapLocation readEnemies(RobotController rc, MapLocation curr, boolean useSpawns, Explore explore) throws GameActionException {
        ...
        if ((useSpawns || bestLoc == null) && bestScore < SPAWN_SCORE) {
            ...
            for (int i = 3; i-->0;) {
                if (enemyFlagSpawns[i] != null && !taken[i]) {
                    dist = StrictMath.sqrt(curr.distanceSquaredTo(enemyFlagSpawns[i]));
                    if (dist < nearestDist) {
                        nearestDist = dist;
                        nearest = enemyFlagSpawns[i];
                    }
                }
            }
            ...
            if (!full) {
                ...
                for (MapLocation bfl : bfls) {
                    dist = StrictMath.sqrt(curr.distanceSquaredTo(bfl)) + 10;
                    if (dist < nearestDist) {
                        nearest = bfl;
                        nearestDist = dist;
                        isBroadcast = true;
            ...
            if (nearest != null) {
                rc.setIndicatorString("use spawn");
                if (isBroadcast) {
