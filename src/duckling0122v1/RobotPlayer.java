...
public strictfp class RobotPlayer {
    ...
    static final int NUM_HEALERS = 20;
    ...
    public static void turn(RobotController rc) throws GameActionException {
        ...
        if (numEnemies > 0 && round >= GameConstants.SETUP_ROUNDS - READY_ROUNDS) {
            ...
            if (isBuilder && !isAttacked && rc.isActionReady() && rc.getCrumbs() >= TrapType.STUN.buildCost + 50) {
                findAndBuild(rc);
            }
            ...
            if (rc.isMovementReady()) {
                if (defendSpot == null && nearestFlagHolder != null) {
                    micro.doMicro(nearbyRobots, nearestFlagHolder, stunned0 + stunned1 + stunned2 + stunned3);
                } else {
    ...
    public static boolean tryReturnBase(RobotController rc) throws GameActionException {
        ...
        if (rc.hasFlag() && isThreatened && nearestAlly != null && nearestAlly.distanceSquaredTo(loc) > 2 && nearestSpawn.distanceSquaredTo(loc) > 8) {
    ...
    public static boolean tryBuildStunDiagonals(RobotController rc) throws GameActionException {
        ...
        for (Direction d : allDirections) {
            m = curr.add(d);
            if ((m.x + m.y) % 2 == 0 && m.isWithinDistanceSquared(nearestEnemy, 16)) {
    ...
    public static boolean findAndBuild(RobotController rc) throws GameActionException {
        if (numAllies < 2 || numEnemies < 2) return false;
        ...
        for (MapInfo mi : nearbyMapInfos) {
            if (mi.isPassable() && mi.getTrapType() == TrapType.NONE) {
                ...
                dist = m.distanceSquaredTo(nearestEnemy);
                if (dist < bestDist && dist <= 16 && dist > 2) {
                    bestDist = dist;
                    best = m;
                }
            }
        ...
        if (best != null) {
            ...
            if (BugNavigation.move(rc, best, true, true, nearbyRobots, radius)) {
                tryBuildStunDiagonals(rc);
            }
        }
        return false;
