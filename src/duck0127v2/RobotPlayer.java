...
public strictfp class RobotPlayer {
    ...
    public static void turn(RobotController rc) throws GameActionException {
        ...
        boolean flagStolen = true;
        if (defendSpot != null) {
            checkNeedsDefense(rc);
            if (rc.canSenseLocation(defendSpot)) {
                for (FlagInfo flag : nearbyFlags) {
                    if (flag.getLocation().equals(defendSpot)) {
                        flagStolen = false;
                    }
                }
            }
        ...
        if (numEnemies > 0 && round >= GameConstants.SETUP_ROUNDS - READY_ROUNDS) {
            ...
            /*if (defendSpot != null && nearestBuilder == null && numEnemies > numAllies && !flagStolen && (!isThreatened || (!isAttacked && rc.getActionCooldownTurns() < 5))) {
                tryBuildDefenses(rc);
            }*/
            ...
            if (roundsMicroNoAction > ROUNDS_MICRO_START_PATHING && rc.canSenseLocation(nearestEnemy) && !hasLineOfSight(rc, curr, nearestEnemy) && rc.isMovementReady()) {
                BugNavigation.move(rc, nearestEnemy, true, isMaxedBuild, nearbyRobots, 4);
            }
