package ineedtodomypdehw0201v2;
...
public strictfp class RobotPlayer {
    ...
    static int turnsSinceMicro = 1000000;
    static MapLocation lastEnemySeen;
    ...
    static String prevEnemyPositions = "";
        ...
        while (true) {
            try {
                ...
                if (!rc.isSpawned()) {
                    ...
                    if (flagCarrying != 0) {
                        Communications.markDropped(rc, flagCarrying);
                        flagCarrying = 0;
                    }
                    BugNavigation.reset();
                    trySpawn(rc);
    ...
    public static void trySpawn(RobotController rc) throws GameActionException {
        ...
        if (round >= GameConstants.SETUP_ROUNDS) {
            MapLocation enemyLoc = Communications.readEnemies(rc, false, explore);
            if (enemyLoc != null) {
                int minDist = 1000000;
    ...
    public static void turn(RobotController rc) throws GameActionException {
        ...
        if (round >= 200) Communications.updateEnemyFlags(rc, enemyFlags);
        if (round < GameConstants.SETUP_ROUNDS) {
        ...
        if (nearestEnemy != null && numAllies > 0) {
            for (RobotInfo info : nearbyAllies) {
                if (info.location.isWithinDistanceSquared(nearestEnemy, GameConstants.VISION_RADIUS_SQUARED)) {
                    numCombatAllies += 1;
                }
            }
        }
        //if (nearestEnemy != null) lastEnemySeen = nearestEnemy;
        ...
        if (numEnemies > 0 && round >= GameConstants.SETUP_ROUNDS - READY_ROUNDS) {
            ...
            long buildCool = StrictMath.round(5.0 * (100.0 + SkillType.BUILD.getCooldown(rc.getLevel(SkillType.BUILD))) / 100.0);
            if (isBuilder && isAttacked && rc.getCrumbs() >= TrapType.STUN.buildCost + 50 && rc.getActionCooldownTurns() < GameConstants.COOLDOWN_LIMIT - buildCool) {
            ...
            if (roundsMicroNoAction > ROUNDS_MICRO_START_PATHING && rc.canSenseLocation(nearestEnemy) && !hasLineOfSight(rc, curr, nearestEnemy) && rc.isMovementReady()) {
                BugNavigation.move(rc, nearestEnemy, true, isMaxedBuild, nearbyEnemies, 4);
            }
            ...
            if (rc.isMovementReady()) {
                ...
                if (rc.isMovementReady()) {
                    if (defendSpot == null && nearestFlagHolder != null) {
                        micro.doMicro(nearbyAllies, nearbyEnemies, nearestFlagHolder, stunned0 + stunned1 + stunned2 + stunned3, true, true);
                    } else {
                        micro.doMicro(nearbyAllies, nearbyEnemies, defendSpot, stunned0 + stunned1 + stunned2 + stunned3, false, nearbyFlags.length > 0);
                    }
                }
        ...
        if (rc.isActionReady()) {
            ...
            for (RobotInfo enemy : nearbyEnemies) {
                if (enemy.location.distanceSquaredTo(curr) <= 10) {
                    heal = false;
                    break;
                }
            }
        ...
        if (flagCarrying > 0) {
            Communications.markTaken(rc, flagCarrying);
            if (rc.senseMapInfo(curr).getSpawnZoneTeamObject() == team) {
                flagCarrying = 0;
            }
        }
    ...
    public static MapLocation getEnemyTarget(RobotController rc, MapLocation loc) throws GameActionException {
        ...
        int allyDmg = (round > 600 ? 210 : 150) / 2;
        //if (info.location == null) System.out.println("WTF INFO");
        if (team.opponent() == null) System.out.println("WTF team");
        RobotInfo[] attackableEnemies = rc.senseNearbyRobots(GameConstants.ATTACK_RADIUS_SQUARED, team.opponent());
        for (RobotInfo info : attackableEnemies) {
            ...
            attackingAllies = rc.senseNearbyRobots(info.location, 10, team);
            

            //score = StrictMath.max(info.health - myDamage, 0) + (allyDmg) / (1+attackingAllies.length) + 1.0/(1+info.attackLevel);
            //score = StrictMath.max(info.health - myDamage - (allyDmg/2) * attackingAllies.length, 0);
            score = 1000.0 / (1+attackingAllies.length) + info.health / 10.0;
            //score = StrictMath.max(0, info.health - myDamage) / (1+attackingAllies.length);
            //if (info.health <= myDamage + allyDmg * attackingAllies.length) score -= allyDmg;
            //if (info.health <= myDamage) score -= allyDmg;
            //if (info.health <= myDamage + allyDmg * attackingAllies.length) score -= 1000;
            if (info.health <= myDamage) score -= 2000;

            if (info.hasFlag) score -= 4000;
    ...
    public static MapLocation getAllyTarget(RobotController rc, MapLocation loc) {
        ...
        double bestScore = 1000000;
        double score;
        int healAmount = rc.getHealAmount();
        ...
        for (RobotInfo info : nearbyAllies) {
            if (rc.canHeal(info.location)) {
                score = StrictMath.min(healAmount, GameConstants.DEFAULT_HEALTH - info.health) * info.health / (1.0 + info.attackLevel);
    ...
    public static boolean tryMoveHeal(RobotController rc) throws GameActionException {
        ...
        int dist;
        for (RobotInfo info : nearbyAllies) {
            if (info.health < GameConstants.DEFAULT_HEALTH - 80) {
                for (int i = 8; i-->0;) {
    ...
    public static boolean tryCollectCrumbs(RobotController rc, boolean fill) throws GameActionException {
        ...
        if (best != null) {
            return BugNavigation.move(rc, best, true, fill, null, null, 0, best.isAdjacentTo(loc));
        }
        return false;
    }
    ...
    public static boolean seekEnemy(RobotController rc) throws GameActionException {
        if (nearestEnemy == null) {
            ...
            /*MapLocation curr = rc.getLocation();
            if (turnsSinceMicro < WAIT_TURNS) {
                MapLocation avoid = lastEnemySeen;
                if (avoid.isWithinDistanceSquared(curr, 20)) {
                    avoid = avoid.add(curr.directionTo(avoid));
                }
                if (!avoid.isWithinDistanceSquared(curr, 20)) {
                    return loc != null && BugNavigation.move(rc, loc, true, isMaxedBuild, null, avoid, 20);
                }
            }*/
            return loc != null && BugNavigation.move(rc, loc, true, isMaxedBuild);
        } else {
            MapLocation loc = Communications.readEnemies(rc, rc.getLocation(), true, explore);
            return loc != null && loc.distanceSquaredTo(rc.getLocation()) > AVOID_DIST && BugNavigation.move(rc, loc, true, isMaxedBuild, nearbyEnemies, 10);
        }
    }
        ...
        if (flagLoc != null) {
            if (loc.isWithinDistanceSquared(flagLoc, GameConstants.INTERACT_RADIUS_SQUARED)) {
                if (rc.canPickupFlag(flagLoc)) {
                    ...
                    flagCarrying = flagID;
                    Communications.markTaken(rc, flagID);
                    return true;
                }
    ...
    public static boolean tryReturnBase(RobotController rc) throws GameActionException {
        ...
        for (MapLocation spawnLoc : spawnLocs) {
            ...
            if (dist < nearestDist) {
                ...
                if (dist <= 2) {
                    Direction d = loc.directionTo(spawnLoc);
                    if (rc.canMove(d)) {
                        rc.move(d);
                        return true;
                    }
        ...
        if (nearestEnemy == null) {
            ...
            ar = StrictMath.min(10, loc.distanceSquaredTo(nearestEnemy));
        }
        return nearestSpawn != null && BugNavigation.move(rc, nearestSpawn, true, true, nearbyEnemies, ar);
    ...
    public static boolean tryDefendFlags(RobotController rc) throws GameActionException {
        ...
        if (nearestFlagHolder != null) {
            if (nfDist <= 2) return tryMoveAway(rc, nearestFlagHolder) || tryReturnBase(rc);
                ...
                for (Direction d : directions) {
                    m = curr.add(d);
                    if (m.isAdjacentTo(nearestFlagHolder) && rc.canFill(m) && (m.x + m.y) % 2 == 0) {
                        rc.fill(m);
                    }
                }
                return true;
    ...
    public static boolean tryBuildDefenses(RobotController rc) throws GameActionException {
        ...
        if (numEnemies <= 2) {
            if (rc.canBuild(TrapType.WATER, defendSpot)) {
                rc.build(TrapType.WATER, defendSpot);
                return true;
            }
        } else {
            MapLocation curr = rc.getLocation();
            MapLocation next = defendSpot.add(defendSpot.directionTo(nearestEnemy));
            if (rc.canBuild(TrapType.STUN, next)) {
    ...
    public static void updateTraps() {
        ...
        for (RobotInfo info : nearbyEnemies) {
            prevEnemyPositions += Integer.toString(info.ID)+info.location;
        }
        //stunned3 = stunned2;
        stunned2 = stunned1;
        stunned1 = stunned0;
    ...
    public static void checkDetonated(RobotController rc) {
        ...
        for (MapInfo mi : nearbyMapInfos) {
            if (mi.getTrapType() == TrapType.NONE && previousTraps.contains(mi.getMapLocation())) {
                ...
                for (RobotInfo info : nearbyEnemies) {
                    dist = det.distanceSquaredTo(info.location);
                    if (dist <= 5) {
                        if (dist <= 4) {
                            stunned0 += info.ID + ",";
                        } else {
                    ...
                    } else if (det.isWithinDistanceSquared(info.location, 13) && prevEnemyPositions.contains(Integer.toString(info.ID))) {
                        int idPos = prevEnemyPositions.indexOf(Integer.toString(info.ID));
                        int start = prevEnemyPositions.indexOf("[", idPos) + 1;
                        int mid = prevEnemyPositions.indexOf(",", idPos);
                        int end = prevEnemyPositions.indexOf("]", idPos);
                        /*if (idPos < 0) System.out.println("idPos WTF?");
                        if (start < 0) System.out.println("start WTF?");
                        if (mid < 0) System.out.println("mid WTF?");
                        if (end < 0) System.out.println("end WTF?");*/
                        //System.out.println(prevEnemyPositions.substring(idPos, end+1));
                        MapLocation prev = new MapLocation(Integer.parseInt(prevEnemyPositions.substring(start, mid)), Integer.parseInt(prevEnemyPositions.substring(mid+2, end)));
                        if (det.isWithinDistanceSquared(prev, 13)) {
                            stunned1 += info.ID + ",";
                        }
                    }
                }
    ...
    public static boolean tryBuildStunDiagonals(RobotController rc) throws GameActionException {
        ...
        if (numCombatAllies < 2 || numEnemies < 2 || !rc.canSenseLocation(nearestEnemy) || !hasLineOfSight(rc, curr, nearestEnemy)) return false;
        for (Direction d : allDirections) {
            m = curr.add(d);
    ...
    public static boolean findAndBuild(RobotController rc) throws GameActionException {
        ...
        if (best != null) {
            ...
            if (BugNavigation.move(rc, best, true, true, nearbyEnemies, radius) && rc.isActionReady()) {
                tryBuildStunDiagonals(rc);
            }
        }
        return false;
    ...
    public static void checkNeedsDefense(RobotController rc) throws GameActionException {
        boolean needsDefense = true;/*nearestBuilder == null || nearestBuilder.distanceSquaredTo(defendSpot) <= 10;*/
    ...
    public static boolean surround(RobotController rc) throws GameActionException {

        MapLocation curr = rc.getLocation();
        MapLocation avoid = lastEnemySeen;
        if (avoid.isWithinDistanceSquared(curr, 20)) {
            avoid = avoid.add(curr.directionTo(avoid));
            if (avoid.isWithinDistanceSquared(curr, 20)) return false;
        }
        
        Direction best = Direction.CENTER;
        int bestDist = curr.distanceSquaredTo(avoid);
        int dist;
        MapLocation m;
        
        for (Direction d : allDirections) {
            m = curr.add(d);
            dist = m.distanceSquaredTo(avoid);
            if (rc.canMove(d) && dist < bestDist && dist > 20) {
                best = d;
                bestDist = dist;
            }
        }
